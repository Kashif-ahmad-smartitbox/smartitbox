...
// Constants
const AUTOSAVE_KEY_PREFIX = "smartitbox:edit-blog:autosave:v1:";
...
export default function EditBlog() {
  ...
  useEffect(() => {
    ...
    const loadBlog = async () => {
      setLoading(true);
      try {
        const res = await getBlog(id, { signal });
        const blog: BlogItem = (res as any).blog;
        ...
        setOriginalStatus(blog.status || null);
        ...
        push({
          title: "Error",
          message: err?.message || "Failed to load blog.",
          variant: "error",
        });
      } finally {
        setLoading(false);
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (formData.cover && !isValidUrl(formData.cover)) {
      errors.cover = "Please enter a valid URL (include https://)";
    }
  ...
  const buildUpdatePayload = useCallback(
    (data: BlogFormData): BlogUpdatePayload => {
      ...
      Object.keys(payload).forEach((key) => {
        const typedKey = key as keyof BlogUpdatePayload;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <div className="mt-4">
            <button
              onClick={() => router.push("/admin/dashboard/content/blogs")}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <label htmlFor="featured" className="text-sm text-gray-700">
                      Feature this post on home / listings
                    </label>
                  </div>
